...
The standard library gave us power,
Then new() came and replaced free().
What price now,
...
At CppCon 2016, Jon Kalb gave Lightning Talk entitled "unsigned: A Guideline for Better Code" in which he argued that we should avoid unsigned integer types in C++. It's a really interesting talk so if you haven't already watched it, please do so now.

I was particularly interested in Jon's talk because I looked into this issue a few years ago and I ended up coming to different conclusions. So I'd like to explain here why Jon's talk hasn't changed my mind and why I still think you should use unsigneds.

A few notes before I begin:

1. I've simplified Jon's point a bit: he's happy with unsigneds for situations like bit-masks, he's only arguing against their use for quantities.
2. I'm aware that aspects of this could easily slip into something like wars over white-space, ie largely unresolvable aesthetic disputes that just waste everyone's time. Still, I think other aspects are genuinely worth discussing.
3. As I perceive it, the weight of experts' opinion is against unsigneds. For example: Jon's someone who really knows what he's talking about, the C++ core guidelines (ES. 102) are with him, I believe the Google C++ style guide's with him. So I recognise the overwhelming likelihood that I'm wrong. But still, I think there are sensible responses to make to Jon's talk so, even if though they're likely to be wrong, I think it's worth me putting those points to move the public conversation forward so we all better understand why.



Let's begin with a plot spoiler. I'm going to make two main points
1. That the problems Jon highlights in his talk are better dealt with by compiler errors than by dropping unsigneds and
2. that once we've done that, the benefits of unsigneds outweigh the remaining costs
...
Here, we've got a function f that takes an unsigned int and then we're calling with -1. This invokes a silent conversion of -1 to an unsigned int, which turns it into a huge positive number, which is what then gets printed.
...
...here, we'd expect the expression in brackets to evaluate to true because -1 *is* less than 1,  but it doesn't because the less-than comparison again invokes a silent conversion from -1 to an unsigned int, which again, turns it into a huge positive number.

Jon's central argument is that to avoid surprising and dangerous behaviour, we just putting unsigned types in our code.

But that seems like the wrong response. It's neither necessary nor sufficient. 
...
...you still get the same surprising result: the call to size() returns an unsigned type, so the compiler silently converts -1 to an unsigned for the comparison and that makes it into a huge positive number.

So if you want to purge the unsigned keyword from your code, you're welcome to, but it doesn't solve this problem, it only sweeps some of the instances of it under the carpet.

Rather than hoping that no contributors to your project ever make this mistake, I think it's better to just tell your compiler to enforce it. Professional Perl developers just expect each other to always use strict and warnings. I propose that professional C++ developers should already expect each other to be making warnings into errors and should now also expect -Wsign-compare -Wsign-conversion too.
...
Of course, if you put your always-positive variables in signed types, then you can always write comments to document that for every such variable. Or you might ofen forget. Or the reader might not be able to find your comments. Or you might let them get out of sync.
Or, even if not, the reader might wonder whether they are out of sync. Or the reader might wonder if any of the other ints are just undocumented always-positive ints. Or the reader might get needlessly confused by having to keep switching between different variables' comments. Or the reader might wonder if you're possibly using negatives values as undocumented special values. Or the reader might wonder whether you've remembered to check the value really is positive, and then re-checked later on, and then rechecked later on again, etc. Or the reader might wonder whether your function might accidentally return a negative and what they should do about that. Or they might wonder about happens if a negative value does get into your code.
...
It's possible that sticking to ints makes code run a bit faster, but I'm guessing that's only a substantial effect in a tiny, tiny proportion of code that's hyper performance criical.


Objection 2. Typing
===================

`unsigned int` does require more key presses than `int`. So create a type alias. I use `uint`:
...
Actually, I mostly use size_t because then I don't get narrowing conversions from standard library return types so I can use compiler warnings for narrowing conversions without too much noise.
...
Since the subtraction in the bracketed expression is being done with unsigned ints, the negative answer of -3 comes out as a huge positive integer.
